#' Create envelope plot to poisson regression
#' 
#'@param fit.model Fitted model with binomial errors and logit link function
...
# o comando title("..."). Para outras ligações trocar o termo
# family=poisson por family=poisson(link=identity) no caso de
# ligação identidade ou por family=poisson(link=sqrt) no caso de
# ligação raiz quadada.
#---------------------------------------------------------------#
#

envel_pois <- function(fit.model) {
  ...
  #par(pty="s")
  qqnorm(td,xlab="Percentile from N(0,1)",
         ylab="Residual Component from Deviance", ylim=faixa, pch=16,main="Quantile-quantile normal plot")
  par(new=T)
  #
